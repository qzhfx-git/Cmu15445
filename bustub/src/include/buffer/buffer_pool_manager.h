...
class FrameHeader {  // 帧状态标签
 ...
 private:
  ...
  std::shared_mutex rwlatch_;  // 读写锁

  /** @brief The number of pins on this frame keeping the page in memory. */
  std::atomic<size_t> pin_count_;  // 有多少请求正在使用这个page

  /** @brief The dirty flag. */
  bool is_dirty_;  // 当前管理的page是否被修改

  page_id_t page_id_;  // 放的是哪部分页
  /**
   ...
   */
  std::vector<char> data_;  // 存放page的实际内容
...
class BufferPoolManager {
 public:
  ...
  frame_id_t GetFrameForPage(page_id_t page_id);
